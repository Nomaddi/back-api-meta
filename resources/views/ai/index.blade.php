        ...
        <tbody style="text-align: center">
            @foreach ($ais as $app)
                <tr>
                    ...
                    <td>{{ $app->api_key }}</td>
                    <td>{{ $app->organizacion }}</td>
                    <td>{{ $app->asistente_id }}</td>
                    <td>
                ...
                {{-- Incluye los modales de editar y mostrar para cada registro --}}
                @include('ai.modals.show-modal', ['app' => $app])
                @include('ai.modals.edit-modal', ['app' => $app])
            @endforeach
        </tbody>
    </table>

{{-- Modal de Confirmación de Eliminación --}}
@include('ai.modals.delete-modal')

{{-- modal create --}}
@include('ai.modals.create-modal')
            ...
            $('#delete-btn').click(function() {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        location.reload(); // Opcional: actualiza la página o haz los ajustes necesarios en el DOM
                    },
    ...
    <script>
        $(document).ready(function() {
            $('#createForm').submit(function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('ais.store') }}", // Asegúrate de que esta es la ruta correcta
                    data: formData,
                    success: function(response) {
                        alert("ai creada con éxito"); // Mensaje de éxito
